...
const BroadbandFetch = () => {
  ...
  const { Number1, broadbandOperator } = state || {};  // Correct variable name

  // If required data is missing, redirect back to the Broadband page
  useEffect(() => {
    if (!Number1 || !broadbandOperator) {
      navigate("/Broadband");
    }
  }, [broadbandOperator, Number1, navigate]);
    ...
    <div className="font-poppins min-h-screen bg-white px-4 py-6 sm:hidden">
      ...
      <div className="flex flex-col items-center justify-center bg-white px-4 font-[Poppins] relative">
        <div className="text-center mb-16">
          <h2 className="text-lg text-blue-600 font-semibold">Confirm Broadband Bill Payment</h2>
        ...
        <div className="relative bg-gray-100 pt-10 p-6 rounded-2xl shadow-md w-full max-w-sm text-center">
          ...
          <div className="absolute -top-10 left-1/2 transform -translate-x-1/2">
            <img
              ...
              alt="Broadband Operator"
              className="w-20 h-20 rounded-full border-4 border-white shadow-md"
            />
          </div>

          {/* Display selected broadband operator and number */}
          <h3 className="font-semibold text-gray-800 text-base mb-1 mt-2">{broadbandOperator}</h3>
          <p className="text-gray-600 text-sm mb-2">{Number1}</p>
